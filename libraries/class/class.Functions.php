<?php
	class Functions
	{
		...
		public function get_status_product($table='', $type='',$id, $title_select='Chọn trạng thái'){

			$duan = $this->d->rawQueryOne("select id, status from #_product where and id = ? and type = ?  limit 0,1",array('du-an',$id));
			$rows = $this->d->rawQuery("select trangthai, id from #_".$table." order by id desc");
			$str = '<select id="'.$type.'" name="data['.$type.']"  class="form-control select2"><option value="">'.$title_select.'</option>';
	        foreach($rows as $v)
	        {
	            if($v["id"] == $duan['status']) $selected = "selected";
	            else $selected = "";
