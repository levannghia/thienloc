			            ...
			            <div class="card-body">
		            		<div class="form-group-category row">
				            	<?php if(isset($config['product'][$type]['dropdown']) && $config['product'][$type]['dropdown'] == true) { ?>
						            ...
						            <?php } ?>
									<?php if(isset($config['product'][$type]['status']) && $config['product'][$type]['status'] == true) { ?>
						                <div class="form-group col-xl-6 col-sm-6">
						                    <label class="d-block" for="id_list">Trạng thái</label>
											<select id="status" name="data[status]"  class="form-control select2">
											<option value="">Chon trạng thái</option>
											<option value="1" <?=@$item['status'] == 1 ? 'selected' : ''?> >Sắp triển khai</option>
											<option value="2" <?=@$item['status'] == 2 ? 'selected' : ''?>>Đang triển khai</option>
											<option value="3" <?=@$item['status'] == 3 ? 'selected' : ''?>>Hoàn thành</option>
											</select>
											<!-- <?=$func->get_status_product('status','status',$id)?> -->
						                </div>
						            <?php } ?>
					            <?php } ?>
		                ...
		                <div class="row">
							<?php if(isset($config['product'][$type]['address']) && $config['product'][$type]['address'] == true) { ?>
			                    <div class="form-group col-md-12">
			                        <label class="d-block" for="masp">Địa chỉ:</label>
			                        <input type="text" class="form-control" name="data[address]" id="adress" placeholder="Địa chỉ" value="<?=@$item['address']?>">
			                    </div>
			                <?php } ?>
							<?php if(isset($config['product'][$type]['area']) && $config['product'][$type]['area'] == true) { ?>
			                    <div class="form-group col-md-12">
			                        <label class="d-block" for="masp">Quy mô:</label>
			                        <input type="text" class="form-control" name="data[area]" id="area" placeholder="Quy mô" value="<?=@$item['area']?>">
			                    </div>
			                <?php } ?>
