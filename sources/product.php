<?php  
	...
	$static = $d->rawQueryOne("select id, type, ten$lang, noidung$lang, photo, ngaytao, ngaysua, options from #_static where type = ? limit 0,1",array('gioi-thieu'));
	if($id!='')
	{ 
		/* Lấy sản phẩm detail */
		...
		/* Lấy tien ich */
		$tienIch = $d->rawQuery("select id, photo, noidungvi, tenvi from #_gallery where id_photo = ? and type = 'du-an' and kind='man' and val = 'tien-ich' and hienthi > 0 order by stt,id desc",array($row_detail['id']));

		/* Lấy vi tri*/
		$viTri = $d->rawQueryOne("select photo, noidungvi from #_gallery where id_photo = ? and type = 'du-an' and kind='man' and val = 'vi-tri' and hienthi > 0 order by stt,id desc limit 0,1",array($row_detail['id']));
	...
	else
	{
		...
		/* Lấy tất cả sản phẩm */
		$where = "";
		$where1 = "a.status = 1 and a.type = ? and a.hienthi > 0";
		$where2 = "a.status = 2 and a.type = ? and a.hienthi > 0";
		$where3 = "a.status = 3 and a.type = ? and a.hienthi > 0";
		$params = array($type);
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql1 = "select a.ten$lang, a.id, a.tenkhongdauvi, a.photo,a.mota$lang,a.address,a.area,a.gia from #_product a where $where1 order by a.stt,a.id desc";
		$sql2 = "select a.ten$lang, a.id, a.tenkhongdauvi, a.photo,a.mota$lang,a.address,a.area,a.gia from #_product a where $where2 order by a.stt,a.id desc";
		$sql3 = "select a.ten$lang, a.id, a.tenkhongdauvi, a.photo,a.mota$lang,a.address,a.area,a.gia from #_product a where $where3 order by a.stt,a.id desc";
		$product_stk = $d->rawQuery($sql1,$params);
		$product_tk = $d->rawQuery($sql2,$params);
		$product_ht = $d->rawQuery($sql3,$params);
